...
export default function SchoolProgramsHero() {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      tl.from(badgeRef.current, { opacity: 0, scale: 0.5, y: -30, duration: 0.8, ease: 'back.out(1.7)' })
      .from(titleRef.current?.children || [], { opacity: 0, y: 80, x: -20, stagger: 0.2, duration: 1.2, ease: 'power4.out' }, '-=0.4')
      .from(subtitleRef.current, { opacity: 0, y: 40, duration: 1, ease: 'power3.out' }, '-=0.8')
      .from(statsRef.current?.children || [], { opacity: 0, scale: 0.8, stagger: 0.1, duration: 0.6, ease: 'back.out(1.7)' }, '-=0.6');

      gsap.from(cardsRef.current.filter(Boolean), {
        opacity: 0, y: 120, scale: 0.85, rotateX: 25, rotateY: 10, duration: 1.4, stagger: { amount: 0.8, from: 'start' }, ease: 'power4.out', delay: 0.6, transformPerspective: 1200
      });

      gsap.from(ctaRef.current, { opacity: 0, y: 60, scale: 0.95, duration: 1.2, ease: 'power3.out', delay: 1.4 });
      ...
      const handleGSAPScroll = () => {
        ...
        cardsRef.current.forEach((card, i) => {
          if (card) gsap.to(card, { y: scrolled * (0.02 + i * 0.01), duration: 0.6, ease: 'power2.out' });
        });
      };
  ...
  const handleCardHover = (program: any, isEntering: boolean) => {
    ...
    if ((window as any).gsap && cardsRef.current[program.id]) {
      ...
      if (isEntering) {
        gsap.to(card, { y: -16, scale: 1.03, duration: 0.7, ease: 'power2.out' });
        gsap.to(icon, { scale: 1.2, rotation: 12, duration: 0.7, ease: 'elastic.out(1, 0.5)' });
        gsap.to(arrow, { x: 8, y: -8, rotation: 10, scale: 1.1, duration: 0.5, ease: 'power2.out' });
        gsap.to(accentLine, { scaleX: 1, duration: 0.9, ease: 'power2.out' });
        gsap.to(content, { y: -6, duration: 0.6, ease: 'power2.out' });
        gsap.to(badge, { scale: 1.15, y: -2, duration: 0.5, ease: 'back.out(2)' });
        gsap.to(glow, { opacity: 0.6, scale: 1.2, duration: 0.6, ease: 'power2.out' });
        gsap.to(title, { scale: 1.02, duration: 0.4, ease: 'power2.out' });
      } else {
        gsap.to(card, { y: 0, scale: 1, duration: 0.7, ease: 'power2.out' });
        gsap.to(icon, { scale: 1, rotation: 0, duration: 0.7, ease: 'power2.out' });
        gsap.to(arrow, { x: 0, y: 0, rotation: 0, scale: 1, duration: 0.5, ease: 'power2.out' });
        gsap.to(accentLine, { scaleX: 0, duration: 0.7, ease: 'power2.out' });
        gsap.to(content, { y: 0, duration: 0.6, ease: 'power2.out' });
        gsap.to(badge, { scale: 1, y: 0, duration: 0.5, ease: 'power2.out' });
        gsap.to(glow, { opacity: 0.2, scale: 1, duration: 0.6, ease: 'power2.out' });
  ...
  const getTransform = (factorX: number, factorY: number) => {
    if (!mounted) return 'translate(0px, 0px)';
    const x = (mousePosition.x - window.innerWidth / 2) * factorX;
    const y = (mousePosition.y - window.innerHeight / 2) * factorY;
